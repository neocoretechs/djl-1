 ...
 *
 */
public class YoloV5SSDPostProcess {
	private static boolean DEBUG = false;
	private static boolean DEBUG_VERBOSE = false;
	String name;
	float probability;
	IRectangle box; //upper left x and y, width, height
	 ...
	 */
	static int process_arraysSSD(int[][] output, float[] predictionsArray, float[] props, List<String> labels, 
			DetectedObjects group, float nms_threshold, int validCount, int model_in_w, int model_in_h, float scale_w, float scale_h) {
		...
		for (int i = 0; i < validCount; i++) {
		    ...
		    probs.add((double) props[i]);
		    classNames.add(labels.get(topClassScoreIndex));
		    Rectangle rec = new Rectangle((double)x1,(double)y1,(double)(x2-x1),(double)(y2-y1));
		    bbs.add(rec);
		    if(DEBUG)
		    	System.out.printf("ssd result %2d: (%4d, %4d, %4d, %4d), %4.2f, %s\n", i, x1, y1, x2, y2, props[i], labels.get(topClassScoreIndex));
		}
		group = new DetectedObjects(classNames,probs,bbs);
		return group.getNumberOfObjects();
	}
	/**
	 * INT8 AFFINE <p/>
	 ...
	 */
	static DetectedObjects process_arraysYOLO(int[] classIdArray, int[] indexArray, float[] filterBoxesArray, float[] objProbsArray, List<String> labels, 
		 float nms_threshold, int validCount, int model_in_w, int model_in_h, float scale_w, float scale_h) {
		...
		for(; i < validCount; i++) {
			...
			box.xmin   = (int)(clamp(x1, 0, model_in_w) / scale_w);
			box.ymin    = (int)(clamp(y1, 0, model_in_h) / scale_h);
			box.xmax  = (int)(clamp(x2, 0, model_in_w) / scale_w);
			box.ymax = (int)(clamp(y2, 0, model_in_h) / scale_h);
		    probs.add((double) obj_conf);
		    classNames.add(labels.get(id));
		    Rectangle rec = new Rectangle((double)box.xmin,(double)box.ymin,(double)(box.xmax-box.xmin),(double)(box.ymax-box.ymin));
		    bbs.add(rec);			
			if(DEBUG)
				System.out.printf("result %2d: (%4d, %4d, %4d, %4d), %s %s\n", i, box.xmin,box.ymin,box.xmax,box.ymax,obj_conf,labels.get(id));
		}
		return new DetectedObjects(classNames,probs,bbs);
	}
	/**
	 * INT8 AFFINE<p/>
	 ...
	 */
	public static DetectedObjects post_process(NDList bufs, int model_in_h, int model_in_w, float conf_threshold,
            float nms_threshold, float scale_w, float scale_h, ArrayList<Integer> qnt_zps,
            ArrayList<Float> qnt_scales, List<String> labels) {
		...
		int validCount0 = 0;
		validCount0 = process(bufs.get(0).toByteArray(), anchor0, grid_h0, grid_w0, model_in_h, model_in_w, stride0, filterBoxes0, objProbs0,
		...
		int validCount1 = 0;
		if(bufs.size() > 1)
			validCount1 = process(bufs.get(1).toByteArray(), anchor1, grid_h1, grid_w1, model_in_h, model_in_w, stride1, filterBoxes1, objProbs1,
		...
		int validCount2 = 0;
		if(bufs.size() > 2)
			validCount2 = process(bufs.get(2).toByteArray(), anchor2, grid_h2, grid_w2, model_in_h, model_in_w, stride2, filterBoxes2, objProbs2,
		...
		if (validCount <= 0) {
			...
			return new DetectedObjects(labels,probs,bbs);
		}
		...
		return process_arraysYOLO(classIdArray, indexArray, filterBoxesArray, objProbsArray, labels, 
				nms_threshold, validCount, model_in_w, model_in_h, scale_w, scale_h);

	}
	/**
	 * FLOAT<p/>
	 * Perform post processing on YOLOv5 type result sets from neural processing unit.<p/>
	 * Data that conforms to the parameters of YOLOv5, in a 3 layer structure if float output.
	 * @param bufs rknn_output[] output layers from NPU
	 * @param model_in_h Model input height
	 * @param model_in_w Model input width
	 * @param conf_threshold confidence threshold
	 ...
	 */
	public static DetectedObjects post_process(NDList bufs, int model_in_h, int model_in_w, float conf_threshold,
            float nms_threshold, float scale_w, float scale_h, List<String> labels) {
		...
		int grid_w0     = model_in_w / stride0;
		int validCount0 = 0;
		float[] fbuf0 = bufs.get(0).toFloatArray();
		...
		int validCount1 = 0;
		if(bufs.size() > 1) {
			float[] fbuf1 = bufs.get(1).toFloatArray();
		...
		int validCount2 = 0;
		if(bufs.size() > 2) {
			float[] fbuf2 = bufs.get(2).toFloatArray();
	...
	public static int post_process(byte[] input0, byte[] input1, float[][] box_priors, int model_in_h, int model_in_w,
            float nms_threshold, float scale_w, float scale_h, ArrayList<Integer> qnt_zps,
            ArrayList<Float> qnt_scales, DetectedObjects group, List<String> labels) {
	 ...
	 */
	public static int post_process(byte[] input0, byte[] input1, float[][] box_priors, int model_in_h, int model_in_w,
            float nms_threshold, float scale_w, float scale_h, DetectedObjects group, List<String> labels) {
